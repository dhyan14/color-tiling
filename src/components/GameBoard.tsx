...
const TetrominoOption: FC<{
  rotation: number;
  isSelected: boolean;
  onClick: () => void;
}> = ({ rotation, isSelected, onClick }) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4">
      ...
      <div className="flex flex-col items-center gap-8">
        ...
        <div className="inline-grid grid-cols-8 gap-0.5 bg-gray-200 p-2 rounded">
          {currentState.grid.map((row, rowIndex) => (
            row.map((cell, colIndex) => {
              const isLastPlaced = lastPlacedCell && 
