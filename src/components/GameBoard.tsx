...
interface TetrominoOptionProps extends HTMLAttributes<HTMLDivElement> {
  rotation: number;
  isSelected: boolean;
  onClick: () => void;
}
...
const TetrominoOption = ({ rotation, isSelected, onClick, ...props }: TetrominoOptionProps) => {
  ...
  return (
    <div className="flex flex-col gap-2 items-center" {...props}>
      <button
        onClick={onClick}
...
export default function GameBoard() {
  ...
  const handleCellClick = (row: number, col: number) => {
    if (isComplete || currentState.piecesPlaced >= MAX_PIECES) {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg p-6 max-w-sm w-full">
          ...
          />
          {passwordError && (
            <p className="text-red-500 mb-4">Incorrect password</p>
          )}
          <button
    ...
    <div className="w-full max-w-4xl mx-auto p-4">
      <div className="mb-8">
        ...
        <p className="text-center text-gray-600 mb-4">
          Place 16 T-shaped tetromino pieces on an 8x8 grid
        </p>
        {errorMessage && (
